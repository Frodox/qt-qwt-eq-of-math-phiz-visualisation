...
/*------------------------------------------------------------------------------*/
// update data fo task A because of increase time
void QwtBeginner::updateDataA()
{
    ...
    uFunCurve->setSamples(data_x, data_u, points_count);

}
/*------------------------------------------------------------------------------*/
// update data for task B because of increase time
...
//    qDebug() << "Need to replot" << t;
    ...
    funPlotArea->setTitle(tr("Пункт А: t = ")
                        + QString::number(t).leftJustified(5, '0', false));
    updateDataA();
    funPlotArea->replot();
...
{
    funPlotArea->setTitle(tr("Пункт B: t = ")
                        + QString::number(t).leftJustified(5, '0', false));
    updateDataB();
    funPlotArea->replot();
